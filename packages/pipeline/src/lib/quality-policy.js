...
const LOW_QUALITY_TITLE_PATTERNS = [
  ...
  /\brolu\b/i,
  /\betkileri\b/i,
  /(?:'nın|'nin|'nun|'nün|in|ın|un|ün)\s+etkisi$/i,
  /(?:'nın|'nin|'nun|'nün|in|ın|un|ün)\s+katkısı$/i,
  /\betkisi$/i,
  /\bkatkısı$/i,
  /\byetenekleri\b/i,
  /\bnedenleri\b/i,
...
const LOW_QUALITY_LANGUAGE_PATTERNS = [
  ...
  /\bbuyuk bir etki\b/i,
  /\bbuyuk bir sekilde\b/i,
  /\bbuyuk etki yaratti\b/i,
  /\bbuyuk katk[ıi]\b/i,
...
const LOW_VALUE_SOURCE_TITLE_PATTERNS = [
  ...
  /\bcrypto wallet\b/i,
  /\bkripto para cüzdanı\b/i,
  /\bkripto para cuzdani\b/i,
  /\bmetamask\b/i,
  /\btaslak/i,
  /\baircraft model\b/i,
...
export function evaluateFactQuality(fact) {
  ...
  if (LOW_QUALITY_TITLE_PATTERNS.some((pattern) => pattern.test(title))) {
