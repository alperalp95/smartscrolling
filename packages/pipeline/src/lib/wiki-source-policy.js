...
const LOW_VALUE_EXTRACT_PATTERNS = [
  ...
  /\bakıllı telefon\b/i,
  /\bandroid cihaz\b/i,
  /\bişletim sistemi\b/i,
  /\bisletim sistemi\b/i,
  /\boperating system\b/i,
  /\bsoftware version\b/i,
  /\btaslak/i,
  /\baircraft model\b/i,
...
const LOCALIZED_CATEGORY_SEED_TOPICS = {
  tr: {
    science: ['Kara delik', 'Büyük Patlama', 'Kuantum mekaniği', 'Görelilik teorisi', 'Doğal seçilim', 'DNA', 'Fosil', 'Kitlesel yok oluş', 'Levha tektoniği', 'Yanardağ', 'Asteroit', 'Periyodik tablo', 'Nöron', 'Süpernova', 'Fotosentez', 'Elektromanyetizma', 'Kozmik mikrodalga arka plan ışıması', 'Kambriyen patlaması', 'Ozon tabakası', 'Marmara Denizi deniz salyası felaketi', 'Karanlık madde', 'Genetik sürüklenme', 'Jeolojik zaman cetveli', 'Higgs bozonu'],
    history: ['Roma İmparatorluğu', 'Moğol İmparatorluğu', 'Sanayi Devrimi', 'Fransız Devrimi', 'Soğuk Savaş', 'Tunç Çağı', 'İpek Yolu', 'Matbaa', 'Magna Carta', 'Rönesans', 'Bizans İmparatorluğu', 'Osmanlı İmparatorluğu', 'Coğrafi keşifler', 'Pompeii', 'Neolitik Devrim', 'Haçlı Seferleri', "İstanbul'un Fethi", 'Mezopotamya', 'Antik Mısır', 'Sümerler', 'Kadeş Muharebesi', 'Kara Ölüm', 'Berlin Duvarı', 'Yazının icadı'],
    philosophy: ['Varoluşçuluk', 'Stoacılık', 'Özgür irade', 'Zihin felsefesi', 'Determinizm', 'Erdem etiği', 'Sokratik yöntem', 'Adalet', 'John Rawls', 'Bilgi felsefesi', 'Ahlak felsefesi', 'Bilinç', 'Nedensellik', 'Rasyonalizm', 'Deneycilik', 'Septisizm', 'Kategorik imperatif', 'İdea', 'Nihilizm'],
    technology: ['İnternet', 'Algoritma', 'Yarı iletken', 'Robotik', 'Mikroişlemci', 'Şifreleme', 'Veritabanı', 'Derleyici', 'Arama motoru', 'Turing makinesi', 'Yapay zekâ', 'Transistör', 'Entegre devre', 'Makine öğrenimi', 'Kriptografi', 'Bilgi kuramı', 'ARPANET', 'Açık kaynak', 'Blok zinciri', 'Kuantum bilgisayarı', 'PRISM (gözetleme programı)'],
    health: ['Bağışıklık sistemi', 'Aşılama', 'Uyku', 'Mikrobiyota', 'Kolesterol', 'Stres', 'Metabolizma', 'İltihap', 'Kardiyovasküler hastalık', 'Antibiyotik', 'Hipertansiyon', 'Diyabet', 'Plasebo', 'Sirkadiyen ritim', 'D vitamini', 'Ağrı', 'Ateş', 'Beslenme', 'Bağırsak florası', 'İnme'],
  },
};

const EXTRA_TR_CATEGORY_SEED_TOPICS = {
  science: [
    'Hücre',
    'RNA',
    'Genetik',
...
function mergeSeedTopics(baseTopics, extraTopics = {}) {
  return Object.fromEntries(
    WIKI_THEME_SEQUENCE.map((category) => [
      category,
      [...new Set([...(baseTopics[category] ?? []), ...(extraTopics[category] ?? [])])],
    ]),
  );
}
...
export function buildWikipediaSeedQueue(limit, lang = 'en') {
  const localizedSeedTopics = LOCALIZED_CATEGORY_SEED_TOPICS[lang] ?? CATEGORY_SEED_TOPICS;
  const seedTopics =
    lang === 'tr'
      ? mergeSeedTopics(localizedSeedTopics, EXTRA_TR_CATEGORY_SEED_TOPICS)
      : localizedSeedTopics;
  const shuffledByCategory = Object.fromEntries(
    WIKI_THEME_SEQUENCE.map((category) => [category, shuffleArray(seedTopics[category] ?? [])]),
  );
