...
export type AiDefinitionResult = {
  word: string;
  definition: string;
  explanation: string;
...
export async function fetchAiDefinition(input: {
  word: string;
  context?: string;
  bookTitle?: string;
}) {
  try {
    const { data, error } = await supabase.functions.invoke('ai-definition', {
      body: {
        word: input.word,
    ...
    return data as AiDefinitionResult;
  } catch (error) {
    const message = await describeFunctionError(error, 'unknown error');
    throw new Error(`ai-definition failed: ${message}`);
  }
}
